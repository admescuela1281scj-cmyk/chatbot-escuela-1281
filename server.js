...
var storage = multer.diskStorage({
  destination: function(_r, _f, cb) { cb(null, uploadDir); },
  filename: function(_r, file, cb) { cb(null, 'cert-' + Date.now() + '-' + Math.round(Math.random() * 1e6) + (path.extname(file.originalname) || '.bin')); }
});
var upload = multer({ storage: storage, fileFilter: function(_r, file, cb) { cb(null, ['image/jpeg','image/png','image/webp','image/gif','application/pdf'].indexOf(file.mimetype) !== -1); }, limits: { fileSize: CONFIG.MAX_FILE_SIZE } });
...
app.get('/', function(_r, res) { res.sendFile(path.join(__dirname, 'public', 'index.html')); });
...
app.post('/api/chat', upload.single('certificate'), function(req, res) {
  ...
  switch (conv.step) {
    ...
    case 'name':
      ...
      conv.data.studentName = text; conv.step = 'grade';
      response = 'Perfecto, ' + text + '! Seleccione el grado:';
      options = GRADES; break;
    case 'grade':
      var idx = parseInt(text) - 1;
      var byIdx = !isNaN(idx) && idx >= 0 && idx < GRADES.length;
      var byNm = null;
      ...
      conv.data.grade = byIdx ? GRADES[idx] : byNm; conv.step = 'shift';
      response = 'Grado: ' + conv.data.grade + '. Seleccione el turno:'; options = SHIFTS; break;
    case 'shift':
      var vs = null;
      for (var s = 0; s < SHIFTS.length; s++) { if (SHIFTS[s].toLowerCase() === text.toLowerCase()) { vs = SHIFTS[s]; break; } }
      if (!vs) { response = 'Seleccione un turno.'; options = SHIFTS; break; }
      conv.data.shift = vs; conv.step = 'section';
      response = 'Turno: ' + vs + '. Seleccione la seccion:'; options = SECTIONS; break;
    case 'section':
      var vsc = null;
      for (var sc = 0; sc < SECTIONS.length; sc++) { if (SECTIONS[sc].toLowerCase() === text.toLowerCase()) { vsc = SECTIONS[sc]; break; } }
      if (!vsc) { response = 'Seleccione A o B.'; options = SECTIONS; break; }
      conv.data.section = vsc; conv.step = 'has_certificate';
      response = 'Seccion: ' + vsc + '. Cuenta con certificado medico?';
      options = ['Si, tengo certificado', 'No, solo justificacion']; break;
    case 'has_certificate':
      var lo = text.toLowerCase();
      var hc = lo.indexOf('si') !== -1 || lo.indexOf('certificado') !== -1;
      var nc = lo.indexOf('no') !== -1 || lo.indexOf('justificacion') !== -1;
    ...
    case 'upload_certificate':
      if (!file && text) {
        ...
        response = 'Adjunte el certificado medico.'; showUpload = true; break;
      }
      if (!file) {
        if (conv.data.certificateFile) { conv.step = 'summary'; showSummary = true; response = buildSummary(conv.data); break; }
        response = 'Esperando certificado...'; showUpload = true; break;
      }
      conv.data.certificateFile = { filename: file.filename, originalname: file.originalname, mimetype: file.mimetype, size: file.size, path: file.path };
    ...
    case 'justification':
      ...
      response = buildSummary(conv.data); break;
    case 'summary':
    ...
    default:
      ...
      response = 'Bienvenido! Cual es el nombre del estudiante?';
  }
...
function buildSummary(data) {
  ...
  s += '\nPresione CONFIRMAR REGISTRO para enviar.';
  return s;
}

function saveSubmission(data, sessionId, callback) {
  var result = { id: 'SUB-' + Date.now(), telegramSent: false, sheetRow: null, fileLink: null };
  var hostUrl = 'https://chatbot-escuela-1281.onrender.com';

  if (data.certificateFile && data.certificateFile.filename) {
    result.fileLink = hostUrl + '/uploads/' + data.certificateFile.filename;
  }
  ...
  if (telegramBot && CONFIG.TELEGRAM_CHAT_ID) {
    ...
    if (result.fileLink) msg += 'Link: ' + result.fileLink + '\n';
